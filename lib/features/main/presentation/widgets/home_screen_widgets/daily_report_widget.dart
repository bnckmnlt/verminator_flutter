...
class _DailyReportWidgetState extends State<DailyReportWidget>
  ...
  IconData systemIcon = FluentIcons.pause_24_filled;
  Color systemColor = Colors.amberAccent;
  ...
  Map<String, dynamic> _getSystemStatusProperties(String status) {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<Map<String, String>>(
      ...
      builder: (context, snapshot) {
        final settings = snapshot.data ?? {"status": "idle"};
        final statusProps =
            _getSystemStatusProperties(settings["status"] ?? "idle");
        final systemColor = statusProps['color'] as Color;
        return Column(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: LinearGradient(
                      colors: [
                        Colors.transparent,
                        systemColor.withAlpha(28),
                        systemColor.withAlpha(24),
                      ],
  ...
  Widget _systemStatusSection(Map<String, dynamic> result) {
    return StreamBuilder<Map<String, String>>(
      stream: _mqttService.systemSettingsStream,
      initialData: _mqttService.lastSystemSettings,
      builder: (context, snapshot) {
        final data = snapshot.data ?? {"status": "idle"};

        final systemStatus =
            (data["status"] ?? "idle").toString().toUpperCase();
        final systemIcon = data["status"] == "feeding"
            ? FluentIcons.food_24_filled
            : data["status"] == "active"
                ? FluentIcons.checkmark_circle_24_filled
                : FluentIcons.pause_24_filled;
        final systemColor = data["status"] == "feeding"
            ? Colors.greenAccent
            : data["status"] == "active"
